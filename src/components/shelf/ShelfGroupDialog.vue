  ...
  <ebook-dialog :title="title" ref="dialog">
    <div class="dialog-list-wrapper" v-if="!ifNewGroup">
      <div class="dialog-list-item"
        ...
        <div class="dialog-list-icon-wrapper" v-if="isInGroup && shelfCategory  && item.id ? shelfCategory .id === item.id : false">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      moveToGroup(group) {
        // 选中的电子书移出 vuex shelfList 移入选中的目录
        this.setShelfList(this.shelfList
          .filter(book => {
            // 如果是在目录中移出电子书 到另外的目录 需要将此目录中这本书删除
            if (book.itemList) {
      ...
      createNewGroup() {
        ...
        // 从 vuex shelfList 移除添加书籍的 对象
        const list = removeAddFromShelf(this.shelfList)
        //  向 vuex shelfList 添加添加目录的对象
        list.push(group)
        //  向 vuex shelfList 添加添加书籍的对象
        this.setShelfList(appendAddToShelf(list)).then(() => {
          this.onComplete()
        })
        // 保存到localStorage 的 shelfList
        // this.onComplete()
        // 将选中的书籍移动到新增的目录中去
